  ...
  return (
    <aside style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px' }}>
        <Section title="Typing" users={typingUsers} countColor="#6366f1" onSelect={setSelectedUser} />
        <Section title="Online" users={activeUsers} onSelect={setSelectedUser} />
        <Section title="Away" users={awayUsers} countColor="#f59e0b" onSelect={setSelectedUser} />
        <Section title="Offline" users={offlineUsers} onSelect={setSelectedUser} />
      </div>
...
const UserItem = ({ user, onSelect }: { user: User; onSelect: (username: string) => void }) => (
  <div 
    onClick={(e) => {
        e.stopPropagation();
...
const Section = ({ title, users, countColor, onSelect }: { title: string; users: User[]; countColor?: string; onSelect: (username: string) => void }) => {
  ...
  return (
    <div style={{ marginBottom: '20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {users.map((user) => <UserItem key={user.username} user={user} onSelect={onSelect} />)}
      </div>
    </div>
  );
};
